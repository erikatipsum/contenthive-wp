...
class PSP_Wordpress_Creation_Kit{
	...
	function wck_output_form_field( $meta, $details, $value = '', $context = '' ){
		...
		if($details['type'] == 'upload'){
			...
			$element .= '<div id="'. esc_attr( $upload_info_div_id ) .'" class="upload-field-details">';
				
				if(isset($thumbnail))
					$element .= $thumbnail;

				if(isset($file_name))
					$element .= '<p><span class="file-name">'. $file_name .'</span>';

				if(isset($file_type))
					$element .= '<span class="file-type">'. $file_type . '</span>';
		...
		<div id="<?php echo $meta ?>" style="padding:10px 0;" <?php if( $this->args['single'] ) echo 'class="single"' ?>>
			<ul class="mb-list-entry-fields">
				...
				<li style="overflow:visible;">
					<a href="javascript:void(0)" class="button-primary" onclick="addMeta('<?php echo esc_js($meta); ?>', '<?php echo esc_js($post->ID); ?>', '<?php echo esc_js($nonce); ?>')"><span>Add Slide</span></a>
				</li>
			</ul>
		</div>
		<?php
	...
	{	
		if( !empty($this->args['meta_name']) && !empty($_GET["meta_name"]) && $this->args['meta_name'] == $_GET["meta_name"] ){			
