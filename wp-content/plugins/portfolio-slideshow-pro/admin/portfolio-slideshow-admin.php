...
if ( PHP_VERSION > 5 ) { //don't bother showing this error message unless the server is running PHP5 or above
	if ( empty( $options['license'] ) ) {
		$psp_errors[] = __( 'The plugin will not be registered for auto-updates until you enter your email address in the "Advanced" section below.', 'portfolio-slideshow-pro' ); 
	}
}
*/
...
$license = get_option('portfolio_slideshow_pro_license_key');
$license_status  = get_option('portfolio_slideshow_pro_license_status');
...
$active_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : 'settings'; ?>
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper">
			<?php _e( 'Portfolio Slideshow Pro', 'portfolio-slideshow-pro' );?>
			<a href="<?php echo add_query_arg('tab', 'settings', remove_query_arg('settings-updated')); ?>" class="nav-tab <?php echo $active_tab == 'settings' ? 'nav-tab-active' : ''; ?>"><?php _e('Slideshow Settings', 'portfolio-slideshow-pro'); ?></a>
			<a href="<?php echo add_query_arg('tab', 'documentation', remove_query_arg('settings-updated')); ?>" class="nav-tab <?php echo $active_tab == 'documentation' ? 'nav-tab-active' : ''; ?>"><?php _e('Documentation', 'portfolio-slideshow-pro'); ?></a>
		</h2><!-- .nav-tab-wrapper -->
		...
		<form action="options.php" method="POST">
		<?php settings_fields('portfolio_slideshow_pro_license'); ?>

			<h3><?php _e('License Activation'); ?></h3>
					...
					<table class="form-table">
						...
						<tbody>
							...
							<tr valign="top">
								...
								<td>
									<input id="portfolio_slideshow_pro_license_key" name="portfolio_slideshow_pro_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" />
									<label class="description" for="portfolio_slideshow_pro_license_key"><?php _e('Enter your license key'); ?></label>
								</td>
							</tr>
							...
							<tr valign="top">	
								...
								<td><?php if( $license_status !== false && $license_status == 'valid' ) { ?>
									<span style="color:green;"><?php _e('active'); ?></span>
									<?php wp_nonce_field( 'portfolio_slideshow_pro_license_nonce', 'portfolio_slideshow_pro_license_nonce' ); ?>
									<input type="submit" class="button-secondary" name="portfolio_slideshow_pro_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
								<?php } else {
									wp_nonce_field( 'portfolio_slideshow_pro_license_nonce', 'portfolio_slideshow_pro_license_nonce' ); ?>
									<input type="submit" class="button-secondary" name="portfolio_slideshow_pro_license_activate" value="<?php _e('Activate License'); ?>"/>
								<?php } ?>
								</td>
							</tr>
	...
	<?php elseif($active_tab == 'documentation') : ?>
		...
		<p>You're using Portfolio Slideshow Pro, by <a href="http://momnt.co">Momnt</a>. Head to our <a href="http://momnt.co/support/portfolio-slideshow-pro/" target="_blank">support forums</a> if you have any questions</a>!</p>	<?php
}
...
function portfolio_slideshow_pro_admin_init(){
	...
	register_setting('portfolio_slideshow_pro_license', 'portfolio_slideshow_pro_license_key', 'portfolio_slideshow_pro_sanitize_license' );
	...
	add_settings_field(
		'portfolio_slideshow_crop',
		__( 'Crop images: <span class="vtip" title="Force the images to crop to the exact dimensions specified. This setting only affects custom image sizes and sizes set directly in the shortcode.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_crop_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_display'
	);
	...
	add_settings_field(
		...
		__( 'Transition FX <span class="vtip" title="You can override these in the shortcode with any option that the jQuery Cycle plugin supports.">?</span>', 'portfolio-slideshow-pro' ),
	...
	add_settings_field(
		'portfolio_slideshow_metapos',
		__( 'Meta content position position', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_metapos_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_display'
	);	
	...
	add_settings_field(
		'portfolio_slideshow_backgroundimages',
		__( 'Load images in background <span class="vtip" title="Slideshows will load images in the background for better performance when this option is enabled. Disable this if you have a lot of slideshows on a single page or are concerned about bandwidth, and images will only be loaded when the slideshow is advanced.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_backgroundimages_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_behavior'
	);
	...
	add_settings_field(
		...
		__( 'Loop the slideshow <span class="vtip" title="Should the slideshow cycle back to the beginning after it gets to the end, or simply stop?">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_loop_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_behavior'
	);
	...
	add_settings_field(
		...
		__( 'Thumbnail/carousel toggle <span class="vtip" title="Hides thumbnails and carousel by default with an option to show them.">?</span>', 'portfolio-slideshow-pro' ),
	...
	add_settings_field(
		'portfolio_slideshow_togglestate',
		__( 'Initial toggle state <span class="vtip" title="Should the thumbs/carousel be visible by default?">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_togglestate_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_navigation'
	);	
	...
	add_settings_field(
		'portfolio_slideshow_fullscreen',
		__( 'Enable fullscreen slideshows <span class="vtip" title="Adds an icon to launch a full-screen, mobile friendly slideshow.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_fullscreen_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_navigation'
	);	
    /*
	...
	add_settings_field(
		...
		__( 'Load jQuery <span class="vtip" title="If you\'re having trouble with another plugin or theme that is loading jQuery, you can adjust this. For best results, leave this at the default setting.">?</span>', 'portfolio-slideshow-pro' ),
	...
	add_settings_field(
		'portfolio_slideshow_scrollable',
		__( 'Load carousel scripts <span class="vtip" title="If you\'re not using the carousel option, it\'s safe to disable this script.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_scrollable_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_diagnostic'
	);
	...
	add_settings_field(
		'portfolio_slideshow_photoswipe',
		__( 'Load Photoswipe <span class="vtip" title="These scripts are necessary to support the full-screen gallery feature. You can disable them if you are not using full-screen galleries, to prevent these scripts from being loaded.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_photoswipe_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_diagnostic'
	);
	...
	add_settings_field(
		'portfolio_slideshow_legacy',
		__( 'Enable legacy<br />slideshow management <span class="vtip" title="If you have slideshows created before version 1.8 that you need to manage, you can check this box to use the old style <strong>Upload Images</strong> meta box.">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_legacy_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_diagnostic'
	);

	add_settings_field(
		'portfolio_slideshow_gallery',
		__( 'Override [gallery] shortcode<span class="vtip" title="Use the built-in Gallery features of WordPress and override all instances of the [gallery] shortcode with Portfolio Slideshow">?</span>', 'portfolio-slideshow-pro' ),
		'portfolio_slideshow_pro_gallery_input',
		'portfolio_slideshow',
		'portfolio_slideshow_pro_diagnostic'
	);
...
function portfolio_slideshow_pro_crop_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[crop]" value="true" <?php checked( "true", $options['crop'] ); ?> />
...
function portfolio_slideshow_pro_metapos_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<select name="portfolio_slideshow_options[metapos]" value="<?php echo $options['metapos']; ?>" />
		<option value="top" <?php if ($options['metapos'] == 'top' ) echo " selected='selected'";?>><?php _e( 'top', 'portfolio-slideshow-pro' );?></option>
		<option value="middle" <?php if ($options['metapos'] == 'middle' ) echo " selected='selected'";?>><?php _e( 'middle', 'portfolio-slideshow-pro' );?></option>
		<option value="bottom" <?php if ($options['metapos'] == 'bottom' ) echo " selected='selected'";?>><?php _e( 'bottom', 'portfolio-slideshow-pro' );?></option>
		<option value="disabled" <?php if ($options['metapos'] == 'disabled' ) echo " selected='selected'";?>><?php _e( 'disabled', 'portfolio-slideshow-pro' );?></option>
	</select>
<?php }
...
function portfolio_slideshow_pro_backgroundimages_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[backgroundimages]" value="true" <?php checked( "true", $options['backgroundimages'] ); ?> />
 
<?php }

function portfolio_slideshow_pro_loop_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
...
function portfolio_slideshow_pro_click_input() {
	...
	<select id="portfolio_slideshow_options_click" name="portfolio_slideshow_options[click]" value="<?php echo $options['click']; ?>" />
		...
		<option value="fullscreen" <?php if ( $options['click'] == "fullscreen" ) echo " selected='selected'";?>><?php _e( 'Opens the image in a fullscreen slideshow', 'portfolio-slideshow-pro' );?></option>
...
function portfolio_slideshow_pro_click_target_input() {
	...
	<select name="portfolio_slideshow_options[target]" value="<?php echo $options['target']; ?>" />
		<option value="_self" <?php if ( $options['target'] == "_self" ) echo " selected='selected'";?>><?php _e( 'Same window', 'portfolio-slideshow-pro' );?></option>
		<option value="_blank" <?php if ( $options['target'] == "_blank" ) echo " selected='selected'";?>><?php _e( 'New window', 'portfolio-slideshow-pro' );?></option>
		<option value="_parent" <?php if ( $options['target'] == "_parent" ) echo " selected='selected'";?>><?php _e( 'Parent window', 'portfolio-slideshow-pro' );?></option>
	</select>
<?php }
...
function portfolio_slideshow_pro_navpos_input() {
	...
	<select name="portfolio_slideshow_options[navpos]" value="<?php echo $options['navpos']; ?>" />
		...
		<option value="middle" <?php if ($options['navpos'] == 'middle' ) echo " selected='selected'";?>><?php _e( 'middle', 'portfolio-slideshow-pro' );?></option>
...
function portfolio_slideshow_pro_pagerstyle_input() {
	...
	<select id="portfolio_slideshow_options_pagerstyle" name="portfolio_slideshow_options[pagerstyle]" value="<?php echo $options['pagerstyle']; ?>" />
		...
		<option value="titles" <?php if ( $options['pagerstyle'] == "titles" ) echo " selected='selected'";?>><?php _e( 'titles', 'portfolio-slideshow-pro' );?></option>
	</select>
<?php }
...
function portfolio_slideshow_pro_togglethumbs_input() {
	...
	<input id='togglethumbs' type="checkbox" name="portfolio_slideshow_options[togglethumbs]" value="true" <?php checked( "true", $options['togglethumbs'] ); ?> />
 
<?php }

function portfolio_slideshow_pro_togglestate_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<select id='togglestate' name="portfolio_slideshow_options[togglestate]" value="<?php echo $options['togglestate']; ?>" />
		<option value="open" <?php if ( $options['togglestate'] == "open" ) echo " selected='selected'";?>><?php _e( 'open', 'portfolio-slideshow-pro' );?></option>
		<option value="closed" <?php if ( $options['togglestate'] == "closed" ) echo " selected='selected'";?>><?php _e( 'closed', 'portfolio-slideshow-pro' );?></option>
	</select>
<?php }
...
function portfolio_slideshow_pro_pagerpos_input() {
	...
	<select name="portfolio_slideshow_options[pagerpos]" value="<?php echo $options['pagerpos']; ?>" />
		...
		<option value="middle" <?php if ( $options['pagerpos'] == 'middle' ) echo " selected='selected'";?>><?php _e( 'middle', 'portfolio-slideshow-pro' );?></option>
...
function portfolio_slideshow_pro_fullscreen_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[fullscreen]" value="true" <?php checked( "true", $options['fullscreen'] ); ?> />
...
function portfolio_slideshow_pro_license_input() {
	...
	echo "<input name='portfolio_slideshow_options[license]'";

	if ( !empty( $options['license'] ) ) {
		echo "type='password'";
	}

	echo " size='50' value='$options[license]' /><span><br />" . __( 'The email address you used to purchase the plugin is your license key to enable automatic updates.', 'portfolio-slideshow-pro' ) . "</span>";
...
function portfolio_slideshow_pro_scrollable_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[scrollable]" value="true" <?php checked( "true", $options['scrollable'] ); ?> />
...
function portfolio_slideshow_pro_photoswipe_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	<input type="checkbox" name="portfolio_slideshow_options[photoswipe]" value="true" <?php checked( "true", $options['photoswipe'] ); ?> />
<?php }
...
function portfolio_slideshow_pro_legacy_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[legacy]" value="true" <?php checked( "true", $options['legacy'] ); ?> />
 
<?php }


function portfolio_slideshow_pro_gallery_input() {
	$options = get_option( 'portfolio_slideshow_options' );?>
	
	<input type="checkbox" name="portfolio_slideshow_options[gallery]" value="true" <?php checked( "true", $options['gallery'] ); ?> />
...
function portfolio_slideshow_pro_validate_options( $input ) {
  ...
  if ( ! $input['carouselsize'] )
	...
	if ( ! isset( $input['crop'] ) )
	  $input['crop'] = null;
		$input['crop'] = ( $input['crop'] == "true" ? "true" : "false" );
	...
	if ( ! isset( $input['backgroundimages'] ) )
	  $input['backgroundimages'] = null;
		$input['backgroundimages'] = ( $input['backgroundimages'] == "true" ? "true" : "false" );
	...
	if ( ! isset( $input['fullscreen'] ) )
	$input['fullscreen'] = null;
	$input['fullscreen'] = ( $input['fullscreen'] == "true" ? "true" : "false" );	
	...
	if ( ! isset( $input['photoswipe'] ) )
    $input['photoswipe'] = null;
	$input['photoswipe'] = ( $input['photoswipe'] == "true" ? "true" : "false" );
    ...
    $input['legacy'] = null;
	$input['legacy'] = ( $input['legacy'] == "true" ? "true" : "false" );

	if ( ! isset( $input['gallery'] ) )
    $input['gallery'] = null;
	$input['gallery'] = ( $input['gallery'] == "true" ? "true" : "false" );
	
	if ( ! isset( $input['debug'] ) )
    $input['debug'] = null;
	$input['debug'] = ( $input['debug'] == "true" ? "true" : "false" );

	if ( ! isset( $input['scrollable'] ) )
    $input['scrollable'] = null;
	$input['scrollable'] = ( $input['scrollable'] == "true" ? "true" : "false" );
...
function portfolio_slideshow_pro_sanitize_license($new) {
	$old = get_option( 'portfolio_slideshow_pro_license_key' );
	if( $old && $old != $new ) {
		delete_option( 'portfolio_slideshow_pro_license_status' ); // new license has been entered, so must reactivate
	}
	return $new;
}

function portfolio_slideshow_pro_activate_license() {


	// listen for our activate button to be clicked
	if( isset( $_POST['portfolio_slideshow_pro_license_activate'] ) ) {

		// run a quick security check 
	 	if( ! check_admin_referer( 'portfolio_slideshow_pro_license_nonce', 'portfolio_slideshow_pro_license_nonce' ) ) 	
		...
		$api_params = array( 
			...
			'item_name' => urlencode( MOMNT_PSP_ITEM_NAME ) // the name of our product in EDD
		);
		
		// Call the custom API.
		$response = wp_remote_get( add_query_arg( $api_params, MOMNT_PSP_STORE_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
		...
		update_option( 'portfolio_slideshow_pro_license_status', $license_data->license );

	}
}
add_action('admin_init', 'portfolio_slideshow_pro_activate_license');

function portfolio_slideshow_pro_deactivate_license() {

	// listen for our activate button to be clicked
	if( isset( $_POST['portfolio_slideshow_pro_license_deactivate'] ) ) {
		...
		// retrieve the license from the database
		$license = trim( get_option( 'portfolio_slideshow_pro_license_key' ) );
		...
		if( $license_data->license == 'deactivated' )
			delete_option( 'portfolio_slideshow_pro_license_status' );

	}
}
add_action('admin_init', 'portfolio_slideshow_pro_deactivate_license');
